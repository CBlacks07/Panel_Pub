...
drop policy if exists "Only service role can manage plans" on public.plans;

create policy "Anyone can view plans" on public.plans
  for select using (true);

create policy "Only service role can manage plans" on public.plans
...
drop policy if exists "Only service role can manage config" on public.app_config;

create policy "Anyone can view config" on public.app_config
  for select using (true);

create policy "Only service role can manage config" on public.app_config
  for all using (auth.role() = 'service_role');
...
drop policy if exists "Anyone can insert rating" on public.shop_ratings;

create policy "Anyone can view ratings" on public.shop_ratings
  for select using (true);

create policy "Anyone can insert rating" on public.shop_ratings
...
drop policy if exists "Vendors can insert own edits" on public.product_edits;
drop policy if exists "Vendors can view own edits"   on public.product_edits;

create policy "Vendors can insert own edits" on public.product_edits
  for insert with check (auth.uid() = user_id);

create policy "Vendors can view own edits" on public.product_edits
  for select using (auth.uid() = user_id);

-- ── 7. RLS POLICIES SUR users ─────────────────────────
drop policy if exists "Public can view shop profiles" on public.users;
...
-- ── 8. RLS POLICIES SUR product_variations ────────────
drop policy if exists "Vendors can insert own variations" on public.product_variations;
drop policy if exists "Vendors can update own variations" on public.product_variations;
drop policy if exists "Vendors can delete own variations" on public.product_variations;
...
-- ── 9. TRIGGER compteur total_articles_created ────────
...
-- ── 10. TRIGGER last_edited_at auto-update ────────────
